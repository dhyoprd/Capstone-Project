<div class="row">
	<div class="col-sm-12">
        ...
        <div class="col-md-2">
            <select class="select2 form-control" name="entry_medicine_type[]" required>
                <option data-tokens=""><?php echo get_phrase('select_medicine_type'); ?></option>
                <?php
                // Mengambil data kategori obat dari database
                $med_category = $this->db->get('med_category')->result_array();
                // Looping melalui setiap kategori obat yang diambil dari database
                foreach ($med_category as $row):
                    $selected = ($row['med_category_id'] == $prescription_entry->medicine_type) ? 'selected' : '';
                ?>
                    <option value="<?php echo $row['med_category_id']; ?>" <?php echo $selected; ?>><?php echo $row['name']; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
                        ...
                        <div id="two_doc_entries">
                            ...
                            <div class="col-md-2">
                            <select class="select2 form-control" input name="entry_medicine_type[]" required>
                                
                                <option data-tokens=""><?php echo get_phrase('select_medicine_type'); ?></option>
                                <?php
                                   $med_category = $this->db->get('med_category')->result_array();  // Mengambil data obat dari database
                                    foreach ($med_category as $row):  // Looping melalui setiap obat yang diambil dari database
                                       ?>
                                   <option value="<?php echo $row['med_category_id']; ?>"><?php echo $row['name']; ?></option>  // Menampilkan nama obat dalam opsi dropdown
                               <?php endforeach; ?>
                           </select>
                            </div>
